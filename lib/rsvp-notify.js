import nodemailer from 'nodemailer';

export const ATTENDANCE_LABELS = {
  yes: 'Absolutely, yes!',
  no: 'Bummer, no.',
  maybe: 'Still deciding, may be?',
};

export const PREDICTION_LABELS = {
  boy: 'Team Boy 💙 (Wear Blue)',
  girl: 'Team Girl 💗 (Wear Pink)',
};

const PREDICTION_SHORT = {
  boy: 'Team Boy 💙',
  girl: 'Team Girl 💗',
};

function buildEmailSubject(data) {
  const attendance = ATTENDANCE_LABELS[data.attendance] || data.attendance;
  const prediction = PREDICTION_SHORT[data.prediction] || data.prediction;
  return `RSVP: ${data.name} — ${attendance} — ${prediction}`;
}

function attendanceEmailStyles(attendance) {
  if (attendance === 'no') {
    return {
      headerBg: 'linear-gradient(135deg, #e85d5d, #c53030)',
      headerColor: '#fff8f8',
      headerTitle: 'RSVP — Can\'t Make It',
      attendanceColor: '#c53030',
      attendanceWeight: 'bold',
    };
  }

  return {
    headerBg: 'linear-gradient(135deg, #f4b942, #d99a1a)',
    headerColor: '#2c2416',
    headerTitle: '🐝 New RSVP Received',
    attendanceColor: '#2c2416',
    attendanceWeight: 'normal',
  };
}

function escapeHtml(str) {
  return String(str)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;');
}

function formatHeadCount(data) {
  if (data.attendance === 'no') return 'Not attending';

  if (data.adults != null || data.kids != null) {
    const adults = data.adults ?? '0';
    const kids = data.kids ?? '0';
    return `${adults} adult${adults === '1' ? '' : 's'}, ${kids} kid${kids === '1' ? '' : 's'}`;
  }

  return data.guests ?? '—';
}

export function formatRsvpText(data) {
  return [
    'New RSVP — Gender Reveal',
    '',
    `Name: ${data.name}`,
    `RSVP: ${ATTENDANCE_LABELS[data.attendance] || data.attendance}`,
    `Prediction: ${PREDICTION_LABELS[data.prediction] || data.prediction}`,
    `Head count: ${formatHeadCount(data)}`,
    '',
    'June 19 · 6:30 PM · 231 Club house Drive, Delaware, OH 43015',
  ].join('\n');
}

export function buildEmailHtml(data) {
  const attendanceLabel = ATTENDANCE_LABELS[data.attendance] || data.attendance;
  const styles = attendanceEmailStyles(data.attendance);

  return `
    <div style="font-family: Georgia, serif; max-width: 560px; margin: 0 auto; color: #2c2416;">
      <div style="background: ${styles.headerBg}; padding: 24px; border-radius: 12px 12px 0 0; text-align: center;">
        <h1 style="margin: 0; color: ${styles.headerColor}; font-size: 24px;">${styles.headerTitle}</h1>
        <p style="margin: 8px 0 0; color: ${styles.headerColor}; opacity: 0.85;">Gender Reveal — Himani & Praneeth</p>
      </div>
      <div style="background: #fdf8ee; padding: 28px; border-radius: 0 0 12px 12px; border: 1px solid #e8dfd0;">
        <table style="width: 100%; border-collapse: collapse;">
          <tr><td style="padding: 10px 0; font-weight: bold; width: 140px;">Name</td><td style="padding: 10px 0;">${escapeHtml(data.name)}</td></tr>
          <tr><td style="padding: 10px 0; font-weight: bold;">RSVP</td><td style="padding: 10px 0; color: ${styles.attendanceColor}; font-weight: ${styles.attendanceWeight};">${attendanceLabel}</td></tr>
          <tr><td style="padding: 10px 0; font-weight: bold;">Prediction</td><td style="padding: 10px 0;">${PREDICTION_LABELS[data.prediction] || data.prediction}</td></tr>
          <tr><td style="padding: 10px 0; font-weight: bold;">Head count</td><td style="padding: 10px 0;">${escapeHtml(formatHeadCount(data))}</td></tr>
        </table>
        <p style="margin-top: 24px; font-size: 13px; color: #6b5d4a; text-align: center;">
          June 19 · 6:30 PM · 231 Club house Drive, Delaware, OH 43015
        </p>
      </div>
    </div>
  `;
}

function createTransporter() {
  const { SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS } = process.env;

  if (!SMTP_HOST || !SMTP_USER || !SMTP_PASS) {
    return null;
  }

  return nodemailer.createTransport({
    host: SMTP_HOST,
    port: Number(SMTP_PORT) || 587,
    secure: Number(SMTP_PORT) === 465,
    auth: {
      user: SMTP_USER,
      pass: SMTP_PASS,
    },
  });
}

export function hasEmailConfig() {
  return Boolean(process.env.RSVP_TO_EMAIL && createTransporter());
}

export function hasNotificationConfig() {
  return hasEmailConfig() || Boolean(process.env.NTFY_TOPIC);
}

export async function sendEmail(data) {
  const toEmail = process.env.RSVP_TO_EMAIL;
  const transporter = createTransporter();

  if (!toEmail || !transporter) {
    return { ok: false, skipped: true };
  }

  await transporter.sendMail({
    from: `"Himani & Praneeth Evite" <${process.env.SMTP_USER}>`,
    to: toEmail,
    subject: buildEmailSubject(data),
    html: buildEmailHtml(data),
    text: formatRsvpText(data),
  });

  return { ok: true };
}

export async function sendNtfy(data) {
  const topic = process.env.NTFY_TOPIC;
  const server = (process.env.NTFY_SERVER || 'https://ntfy.sh').replace(/\/$/, '');

  if (!topic) {
    return { ok: false, skipped: true };
  }

  const response = await fetch(`${server}/${encodeURIComponent(topic)}`, {
    method: 'POST',
    headers: {
      Title: buildEmailSubject(data),
      Tags: 'bee,honeybee',
      Priority: '4',
    },
    body: formatRsvpText(data),
  });

  if (!response.ok) {
    throw new Error(`ntfy notification failed (${response.status})`);
  }

  return { ok: true };
}

export async function deliverRsvpNotifications(data) {
  const results = await Promise.allSettled([sendEmail(data), sendNtfy(data)]);
  const delivered = results.some(
    (result) => result.status === 'fulfilled' && result.value.ok
  );

  if (!delivered) {
    const errors = results
      .filter((result) => result.status === 'rejected')
      .map((result) => result.reason?.message || 'Unknown error');

    throw new Error(errors[0] || 'Failed to deliver RSVP notification.');
  }

  return { success: true };
}
